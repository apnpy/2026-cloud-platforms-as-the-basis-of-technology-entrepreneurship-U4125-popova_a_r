...
# Вывод 
В работе я научилась создавать service account и виртуальную машину в Google Cloud Platform, а также настраивать доступ к Cloud Storage.

Сначала у меня получалось скачивать файлы из бакета. После изменения роли на Compute Viewer доступ должен был пропасть, но он остался. Я разобралась, что причина была в публичном доступе к бакету (allUsers), из-за которого файлы можно было скачивать независимо от ролей.

После удаления публичного доступа копирование перестало работать, и появилась ошибка доступа. Это подтвердило, что права настроены корректно.

В итоге я поняла, как работают роли IAM и настройки доступа на уровне бакета, и как они влияют на доступ к данным.
