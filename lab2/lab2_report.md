...
После этого я удалила сервис:
<img width="1440" height="415" alt="image" src="https://github.com/user-attachments/assets/591bd6a5-262d-40f1-8606-68a3cd5efab2" />
<img width="1440" height="778" alt="image" src="https://github.com/user-attachments/assets/6a73fdac-2bae-49d5-84c9-0b09aa529c1b" />

# Вывод

В ходе работы я научилась разворачивать сервис в Google Cloud Run и настраивать его параметры. Был создан и задеплоен сервис с минимальными ресурсами, после чего проверена его работа через браузер.

Я изучила логи и метрики в Observability: убедилась, что сервис успешно запускается, обрабатывает запросы и работает без ошибок. Далее изменила порт и повторно задеплоила сервис — он продолжил корректно функционировать.

Также я настроила распределение трафика между двумя версиями сервиса (50/50) и убедилась, что нагрузка распределяется равномерно, без ухудшения производительности.

В итоге я разобралась с базовыми возможностями Cloud Run: деплой, мониторинг, изменение конфигурации и управление версиями сервиса.
